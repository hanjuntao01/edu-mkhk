...
class action extends app
{
	...
	private function makescore()
	{
		...
		else
		{
			...
			$this->tpl->assign('sessionvars',$sessionvars);
			$this->tpl->assign('userlevel_last',$userlevel_last);

			if($userlevel_last['userlevel']>=10 && $sessionvars['examsessionscore']>=$sessionvars['examsessionsetting']['examsetting']['passscore'] && $examlevel>=9){
				$this->tpl->display('exam_score_honor');  //总测试及格后页面
			}else{
		...
		if($this->ev->get('insertscore'))
		{
			...
			else
			{
				if($sessionvars['examsessionsetting']['examdecide'])
				{
					...
					$message = array(
						'statusCode' => 200,
						"message" => "Operating successfully, this paper needs teachers to score, please wait for the results",
					    "callbackType" => 'forward',
					...
					$message = array(
						'statusCode' => 300,
						"message" => "Operation failed, please re submit"
					);
				}
				else
				{
	...
	private function selectquestions()
	{
		...
		if($userlevel['userlevel']>=$r['examlevel']){
			if(!$r['examid'])
			{
				$message = array(
					'statusCode' => 300,
					"message" => "Parameter error, trying to quit after re entering"
				);
				$this->G->R($message);
			}
			else
			{
				if($r['examtype'] == 1)
				{
					...
					$message = array(
						'statusCode' => 200,
						"message" => "Operation successfully, enter the examination",
					    "callbackType" => 'forward',
			...
			$message = array(
					'statusCode' => 300,
					"message" => "Please pass the examination in front!"
				);
				$this->G->R($message);
		}
	...
	private function index()
	{
		...
		$this->tpl->assign('sessionvars',$sessionvars);
		$this->tpl->assign('userlevel',$userlevel['userlevel']);
		$this->tpl->assign('number',$number);
